...
function handleTouchMove(e) {
  ...
  // Highlight keep/tray zones
  const keepZone = dom.keep.closest('.keep-zone');
  const trayZone = dom.tray.closest('.tray-zone');
  if (keepZone) keepZone.classList.remove('drag-over-zone');
  if (trayZone) trayZone.classList.remove('drag-over-zone');
  
  if (target && target.classList.contains('drop-slot')) {
    target.classList.add('drag-over');
  } else if (target && (target === dom.keep || dom.keep.contains(target) || (keepZone && keepZone.contains(target)))) {
    if (keepZone) keepZone.classList.add('drag-over-zone');
  } else if (target && (target === dom.tray || dom.tray.contains(target) || (trayZone && trayZone.contains(target)))) {
    if (trayZone) trayZone.classList.add('drag-over-zone');
  }
}
...
function handleTouchEnd(e) {
  ...
  if (target && target.classList.contains('drop-slot') && !target.hasChildNodes()) {
    ...
    if (d && !d.inSlot && !d.held) { d.held = true; renderDice(); }
  } else if (target && (target === dom.tray || dom.tray.contains(target) || (trayZone && trayZone.contains(target)))) {
    // Drop back to tray
    const d = state.dice.find(x => x.id === id);
    if (d && !d.inSlot && d.held) { d.held = false; renderDice(); }
  }
...
function cleanupTouchDrag() {
  if (touchDragData) {
    if (touchDragData.ghost) touchDragData.ghost.remove();
    if (touchDragData.el) touchDragData.el.style.opacity = '1';
    touchDragData = null;
  }
  // 혹시 남아있는 고스트 요소 전부 제거
  document.querySelectorAll('.touch-ghost').forEach(g => g.remove());
  $$('.drop-slot').forEach(s => s.classList.remove('drag-over'));
  const keepZone = dom.keep.closest('.keep-zone');
  const trayZone = dom.tray.closest('.tray-zone');
  if (keepZone) keepZone.classList.remove('drag-over-zone');
...
// 안전장치: 2초마다 고스트가 남아있으면 강제 제거
setInterval(() => {
  const ghosts = document.querySelectorAll('.touch-ghost');
  if (ghosts.length > 0 && !touchDragData) {
...
// Mouse drag: tray <-> keep zone
function setupZoneDrag(zoneEl, targetHeld) {
  zoneEl.addEventListener('dragover', e => e.preventDefault());
  zoneEl.addEventListener('drop', e => {
    e.preventDefault();
    if (!state.draggedEl) return;
    const did = state.draggedEl.dataset.id;
    const d = state.dice.find(x => x.id === did);
    if (d && !d.inSlot) {
      d.held = targetHeld;
      renderDice();
    }
...
setupZoneDrag(dom.keep, true);   // drop into keep -> held
setupZoneDrag(dom.tray, false);  // drop into tray -> unheld
